...
Status Appender::AppendRow(const std::vector<Value>& row) {
  if (row.size() != schema_->column_count())
    return Status(StatusCode::kInvalidArgument, "column count mismatch");
  ...
  for (size_t i = 0; i < row.size(); ++i) {
    ...
    if (type == ColumnType::kFloat && !std::holds_alternative<double>(val))
      return Status(StatusCode::kInvalidArgument,
                    "expected FLOAT for column " + schema_->column_at(i).name);
    if (type != ColumnType::kFloat && !std::holds_alternative<int64_t>(val))
      return Status(StatusCode::kInvalidArgument,
                    "expected INT/TIMESTAMP for column " +
                        schema_->column_at(i).name);
  }
