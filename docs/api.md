...
**WHERE 过滤：** `SELECT ... WHERE ts op val [AND ts op val]` 支持时间过滤，操作符 `>=` `<=` `>` `<`。与 `Select()` 语义等价。WHERE 子句中时间戳字面量精度与列精度按 [精度自适应](#时间戳精度自适应) 规则自动适配。
...
SQL 查询中 `WHERE ts op val [AND ts op val]` 的时间范围过滤，支持 `>=` `<=` `>` `<`。

### `>=` / `>` 下界规则

| 操作符 | 处理 | 示例（列=DAY） |
|--------|------|----------------|
| `>=` 输入细于列 | 截断到列精度周期起点 | `>= 20260101-10:50:00` → `>= 20260101 00:00:00` |
| `>=` 输入等于列 | 不变 | `>= 20260101` → `>= 20260101` |
| `>=` 输入粗于列 | 不变（自然对齐） | `>= 20260101` 在 MICRO 列 → `>= 20260101 00:00:00.000000` |
| `>` 任意精度 | `from_ts = val + 1µs`（不截断） | `> 20260101-10:50:00` → `> 20260101-10:50:00.000001` |
...
| `<` 任意精度 | `to_ts = val - 1µs`（不扩展） | `< 20260101-10:50:00` → `< 20260101-10:49:59.999999` |
