...
-- 时间范围过滤（<= 上界）
SELECT * FROM ticks WHERE ts <= 20260101-10:00:00;
...
**WHERE 精度自适应：** 时间戳字面量精度与建表列精度自动适配（详见 [api.md — 时间戳精度自适应](#)）：

| 示例 | 列精度 | 实际效果 |
|------|--------|----------|
| `>= 20260101-10:50:00` | DAY | 截断为 `>= 20260101`（00:00:00） |
| `>= 20260101` | MICRO | 保持 `>= 20260101-00:00:00`（已对齐） |
| `<= 20260101` | MICRO | 扩展为 `<= 20260101-23:59:59.999999` |
| `<= 20260101-10:50` | MICRO | 扩展为 `<= 20260101-10:50:59.999999` |
